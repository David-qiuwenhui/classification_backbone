...
def main():
    ...
    if not data_cfg.get("train").get("pretrained_flag"):
        model.init_weights()
    if data_cfg.get("train").get("freeze_flag") and data_cfg.get("train").get(
        "freeze_layers"
    ):
    ...
    # 初始化优化器
    optimizer = eval("optim." + optimizer_cfg.pop("type"))(
        params=model.parameters(), **optimizer_cfg
    )
    ...
    runner = dict(
        ...
        iter=0,
        epoch=0,
        max_epochs=data_cfg.get("train").get("epochs"),
        max_iters=data_cfg.get("train").get("epochs") * len(train_loader),
    ...
    if args.resume_from:
        ...
        shutil.copyfile(
            args.config, os.path.join(save_dir, os.path.split(args.config)[1])
        )
        model = init_model(model, data_cfg, device, mode="train")
