...
def get_metrics_output(eval_results, metrics_output, classes_names, indexs, APs):
    ...
    for i in range(len(eval_results.get("confusion"))):
        writer_list.append(
            [classes_names[i]] + [str(x) for x in eval_results.get("confusion")[i]]
        )
...
def get_prediction_output(
    preds, targets, image_paths, classes_names, indexs, prediction_output
):
    nums = len(preds)
...
def plot_ROC_curve(preds, targets, classes_names, savedir):
    ...
    for j in range(cols):
        ...
        PRECISION, RECALL, thresholds = precision_recall_curve(
            targets.tolist(), pre_score, pos_label=j
        )
...
def main():
    ...
    save_dir = os.path.join(
        "eval_results", model_cfg.get("backbone").get("type"), dirname
    )
