...
def parse_args():
    ...
    parser.add_argument("--input", default="./img", type=str, help="input images path")
    parser.add_argument(
        "--config", default="./models/hrnet/hrnet_w32.py", help="config file path"
    )
    parser.add_argument(
        "--skip-type",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        default="./img_out/visual_pipeline",
        type=str,
    ...
    parser.add_argument(
        "--sleep", type=float, default=1, help="time to sleep while display every image"
    )
    parser.add_argument(
        "--show",
        default=False,
...
def main():
    ...
    for img in image_files[: args.number]:
        data = dict(
            img_info=dict(filename=os.path.join(args.input, img)), img_prefix=None
        )
