...
def parse_args():
    ...
    parser.add_argument(
        "--ngpus", type=int, default=1, help="The number of GPUs used in training."
    )
...
def simulate_train(data_loader, data_cfg, optimizer_cfg, lr_config):
    ...
    runner = dict(
        ...
        max_iters=data_cfg.get("train").get("epochs") * len(data_loader),
    )
    ...
    with tqdm(total=runner.get("max_epochs")) as pbar:
        for epoch in range(data_cfg.get("train").get("epochs")):
