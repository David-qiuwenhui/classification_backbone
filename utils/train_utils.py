...
def print_info(cfg):
    backbone = (
        cfg.get("backbone").get("type") if cfg.get("backbone") is not None else "None"
    )
    ...
    loss = (
        cfg.get("head").get("loss").get("type")
        if cfg.get("head").get("loss") is not None
        else "None"
    )
...
def resume_model(
    model, runner, checkpoint, meta, resume_optimizer=True, map_location="default"
):
...
def train(model, runner, lr_update_func, device, epoch, epochs, meta):
    train_loss = 0
    ...
    model.train()
    with tqdm(
        total=len(runner.get("train_loader")),
    ...
    if train_loss / len(runner.get("train_loader")) < runner.get("best_train_loss"):
        ...
        runner["best_train_weight"] = os.path.join(
            meta["save_dir"],
            "Train_Epoch{:03}-Loss{:.3f}.pth".format(
                epoch + 1, train_loss / len(runner.get("train_loader"))
            ),
        )
        meta["best_train_weight"] = runner["best_train_weight"]
        save_checkpoint(
            model, runner.get("best_train_weight"), runner.get("optimizer"), meta
        )


def validation(model, runner, cfg, device, epoch, epochs, meta):
    preds, targets = [], []
    model.eval()
    with torch.no_grad():
        with tqdm(
            total=len(runner.get("val_loader")),
    ...
    if eval_results.get("accuracy_top-1") > runner.get("best_val_acc"):
        ...
        runner["best_val_weight"] = os.path.join(
            meta["save_dir"],
            "Val_Epoch{:03}-Acc{:.3f}.pth".format(
                epoch + 1, eval_results.get("accuracy_top-1")
            ),
        )
        meta["best_val_weight"] = runner["best_val_weight"]
        save_checkpoint(
            model, runner.get("best_val_weight"), runner.get("optimizer"), meta
        )

    if epoch > 0 and os.path.isfile(runner["last_weight"]):
        os.remove(runner["last_weight"])
    runner["last_weight"] = os.path.join(
        meta["save_dir"], "Last_Epoch{:03}.pth".format(epoch + 1)
    )
