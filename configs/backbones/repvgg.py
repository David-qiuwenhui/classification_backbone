...
class RepVGGBlock(BaseModule):
    ...
    def create_conv_bn(self, kernel_size, dilation=1, padding=0):
        ...
        conv_bn.add_module(
            "norm", build_norm_layer(self.norm_cfg, num_features=self.out_channels)[1]
        )
    ...
    def forward(self, x):
        def _inner_forward(inputs):
            ...
            inner_out = (
                self.branch_3x3(inputs) + self.branch_1x1(inputs) + branch_norm_out
            )
    ...
    def _norm_to_conv3x3(self, branch_nrom):
        ...
        conv_weight = torch.zeros(
            (self.in_channels, input_dim, 3, 3), dtype=branch_nrom.weight.dtype
        )
...
class RepVGG(BaseModule):
    ...
    def __init__(
        ...
        for i in range(len(arch["num_blocks"])):
            ...
            out_channels = int(base_channels * 2**i * self.arch["width_factor"][i])
